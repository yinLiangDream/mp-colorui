...
export default function ClTip_H5(props: IProps) {
  ...
  const [id] = useState(
    () => `cl-tip-content-${Math.random().toString().replace('.', '')}`
  )
  const [idMessage] = useState(
    () => `cl-tip-message-${Math.random().toString().replace('.', '')}`
  )
  ...
  useEffect(() => {
    ...
    if (props.direction === 'top') {
      ...
      if (res.left < res.right && res.left < screenWidth / 2) {
        ...
        distance.arrowTransform = `translateX(${
          Math.abs(translateX) + res.width / 2 - 10
        }px) rotate(45deg) translateY(0)`
      } else {
        const maxRight = screenWidth - res.left
        if (maxRight < contentWidth) translateX = contentWidth - maxRight
      ...
      distance.transform = `translateX(${-res.width - 10}px) translateY(${
        -diffHeight / 2 + moveTop - moveBottom
      }px)`
