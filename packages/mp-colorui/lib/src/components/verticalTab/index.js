...
export default function ClVerticalTab(props) {
    ...
    const tabsComponent = tabs.map((item) => (React.createElement(View, { key: item.cu_vertical_tab_id, className: `cu-item ${current === item.id ? 'cur' : ''}`, onClick: () => {
            id = item.id;
            scrollTab = true;
            changeTop(id);
            setCurrent(item.id);
    ...
    const weappComponent = (React.createElement(ScrollView, { scrollY: true, scrollWithAnimation: true, 
    ...
    const h5Component = (React.createElement(ScrollView, { scrollY: true, scrollWithAnimation: true, 
        // scrollAnimationDuration={200}
        style: {
            height: props.height === 'full' ? '100%' : pxTransform(props.height),
            maxHeight: '100vh',
    ...
    return (React.createElement(View, { className: classNames('flex', props.className), style: Object.assign({}, props.style) },
        React.createElement(ScrollView, { scrollY: true, scrollWithAnimation: true, className: 'VerticalNav nav', style: {
                height: props.height === 'full' ? '100%' : pxTransform(props.height),
                maxHeight: '100vh'
