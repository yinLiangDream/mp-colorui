...
export default class Calendar_h5 extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        const weeksComponent = preWeek.length
            ? [preWeek, thisWeek, nextWeek].map(week => (React.createElement(SwiperItem, { key: week[3].date },
                React.createElement(ClGrid, { col: 7 }, week.map(item => (React.createElement(View, { key: item.timeStamp, onClick: () => {
                        this.clickDate({
                            item
                        });
                    } },
            ...
            : '';
        const monthsComponent = [preMonth, thisMonth, nextMonth].every(item => item.length > 0)
            ? [preMonth, thisMonth, nextMonth].map(month => (React.createElement(SwiperItem, { key: month[3].date },
                React.createElement(ClGrid, { col: 7 }, month.map(item => (React.createElement(View, { key: item.timeStamp, onClick: () => {
                        this.clickDate({
                            item
                        });
                    } },
        ...
        return (React.createElement(ClCard, { type: showType },
            React.createElement(View, { className: classNames([BG_COLOR_LIST['white'], 'cuCalendar']) },
                                ...
                                React.createElement(ClGrid, { col: 4 }, months.map((item, index) => (React.createElement(View, { key: item, className: classNames([
                                    ...
                                    React.createElement(ClLayout, { margin: 'small', marginDirection: 'vertical' },
                                        React.createElement(ClButton, { size: 'small', bgColor: 'light-gray', shadow: false, long: true, text: item, onClick: () => {
                                            ...
                                            } }))))))), mode: 'click', width: 300, show: this.state.showMonths, onChange: () => {
                ...
                React.createElement(View, { className: 'week' },
                    ...
                    React.createElement(Swiper, { current: this.state.currentActive, skipHiddenItemLayout: true, circular: true, key: 'key-' + Math.random(), style: {
                            width: '100%',
                            height: calendarType === 'week'
                                ? pxTransform(120)
                                : pxTransform(120 * currentActiveLines)
                        }, onChange: e => {
                            if (this.state.currentActive !== e.detail.current)
                                this.changeDate(e);
