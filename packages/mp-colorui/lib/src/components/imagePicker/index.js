...
export default class ClImagePicker extends React.Component {
    ...
    render() {
        ...
        // const chooseImg = chooseImgObj || {};
        const maxPic = max || 0;
        const imgComponent = imgList.map((item, index) => (React.createElement(View, { className: 'padding-xs bg-img bg-gray', key: item.url, style: { borderRadius: '6px', position: 'relative' }, onClick: () => {
                this.viewImage.call(this, item.url);
            } },
            React.createElement(Image, { src: item.url, mode: 'widthFix', style: {
                    width: '100%',
                    position: 'absolute',
                    left: '50%',
                    top: '50%',
            ...
            item.status === 'none' && !item.disabled ? (React.createElement(View, { className: 'cu-tag bg-red', onClick: e => {
                    e.stopPropagation();
                    this.delImg.call(this, index);
                }, style: {
                    backgroundColor: 'rgba(355, 355, 355, 0.8)'
                } },
                React.createElement(ClIcon, { iconName: 'close', color: 'black', size: 'xsmall' }))) : (''),
            React.createElement(View, { className: 'cu-tag', style: {
                    backgroundColor: 'rgba(355, 355, 355, 0.8)',
                    display: `${item.status === 'fail' ? '' : 'none'}`
                } },
                React.createElement(ClIcon, { iconName: 'warnfill', size: 'xsmall', color: 'red' })),
            React.createElement(View, { className: 'cu-tag', style: {
                    backgroundColor: 'rgba(355, 355, 355, 0.8)',
                    display: `${item.status === 'success' ? '' : 'none'}`
                } },
                React.createElement(ClIcon, { iconName: 'roundcheckfill', size: 'xsmall', color: 'olive' })),
            React.createElement(View, { className: 'cu-tag', style: {
                    backgroundColor: 'rgba(355, 355, 355, 0.8)',
                    display: `${item.status === 'loading' ? '' : 'none'}`
                } },
                React.createElement(View, { className: 'imagePicker-rotate-360' },
                    React.createElement(ClIcon, { iconName: 'loading', size: 'xsmall', color: 'blue' }))))));
        return (React.createElement(View, { className: classNames('cu-form-group', className), style: Object.assign({}, style) },
            React.createElement(View, { className: 'grid col-4 grid-square flex-sub' },
                imgComponent,
                (maxPic === 0 || maxPic !== imgList.length) && (React.createElement(View, { className: 'padding-xs bg-gray', onClick: this.ChooseImage.bind(this), style: { borderRadius: '6px' } },
                    React.createElement(Text, { className: 'cuIcon-cameraadd' }))))));
    }
}
